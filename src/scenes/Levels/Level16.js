class Level16 extends Phaser.Scene {
    ...
    create(){
        ...
        if (Phaser.Math.Between(1,2) == 1) {
            this.player = this.matter.add.sprite(580, 870, '', null, { shape: this.shapes.circle });
            this.arrow = this.physics.add.sprite(720/2, 430, 'arrowp2').setSize(30, 30).setOrigin(-0.31,0.45);
        } else {
            this.player = this.matter.add.sprite(140, 870, '', null, { shape: this.shapes.circle });
        ...
        this.add.text(360, 20, 'No Where but UP', { font: '28px Impact', fill: '#1b2cc2'}).setOrigin(0.5).setDepth(3);
        ...
        this.matter.world.on('collisionstart', function (event, bodyA, bodyB) {
            if ((bodyA.gameObject && bodyA.gameObject.texture.key == 'goal1') || (bodyB.gameObject && bodyB.gameObject.texture.key == 'goal1')) {
                this.player2score++;
                this.player.setPosition(580, 870);
